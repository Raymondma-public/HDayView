...
public class DayViewTest {
    ...
    private HDayView dayView;

    @Before
    public void setup() {
        MockitoAnnotations.initMocks(this);

        when(context.obtainStyledAttributes(attrs, R.styleable.HDayView)).thenReturn(array);
        when(array.getDimensionPixelSize(R.styleable.HDayView_dividerHeight, 0)).thenReturn(
                DIVIDER_HEIGHT);
        when(array.getDimensionPixelSize(R.styleable.HDayView_halfHourHeight, 0)).thenReturn(
                HALF_HOUR_HEIGHT);
        when(array.getDimensionPixelSize(R.styleable.HDayView_hourLabelMarginEnd, 0)).thenReturn(
                HOUR_LABEL_MARGIN_END);
        when(array.getDimensionPixelSize(R.styleable.HDayView_eventMargin, 0)).thenReturn(EVENT_MARGIN);
        when(array.getInt(R.styleable.HDayView_startHour, HDayView.MIN_START_HOUR)).thenReturn(HDayView.MIN_START_HOUR);
        when(array.getInt(R.styleable.HDayView_endHour, HDayView.MAX_END_HOUR)).thenReturn(HDayView.MAX_END_HOUR);

        when(hourLabelView.getMeasuredWidth()).thenReturn(50);
        when(hourLabelView.getMeasuredHeight()).thenReturn(20);

        dayView = new HDayView(context, attrs, 0, false);
        ...
        List<HDayView.EventTimeRange> eventTimeRanges = new ArrayList<>();
        eventTimeRanges.add(new HDayView.EventTimeRange(30, 180));
        eventTimeRanges.add(new HDayView.EventTimeRange(90, 120));
        eventTimeRanges.add(new HDayView.EventTimeRange(150, 300));
        eventTimeRanges.add(new HDayView.EventTimeRange(150, 300));
        ...
        dayView.eventColumnSpansHelper = new HDayView.EventColumnSpansHelper(eventTimeRanges);
        dayView.eventRects.addAll(eventRects);
        dayView.setParentHeight(PARENT_WIDTH);
    }
    ...
    public void timeRanges() {
        HDayView.EventTimeRange range = new HDayView.EventTimeRange(20, 40);

        assertThat(range.conflicts(new HDayView.EventTimeRange(5, 15)), is(false));
        assertThat(range.conflicts(new HDayView.EventTimeRange(50, 90)), is(false));
        assertThat(range.conflicts(new HDayView.EventTimeRange(5, 20)), is(false));
        assertThat(range.conflicts(new HDayView.EventTimeRange(40, 90)), is(false));

        assertThat(range.conflicts(new HDayView.EventTimeRange(20, 40)), is(true));
        assertThat(range.conflicts(new HDayView.EventTimeRange(10, 60)), is(true));
        assertThat(range.conflicts(new HDayView.EventTimeRange(25, 35)), is(true));
        assertThat(range.conflicts(new HDayView.EventTimeRange(10, 35)), is(true));
        assertThat(range.conflicts(new HDayView.EventTimeRange(25, 50)), is(true));
    }

    @Test
    public void singleEventColumnSpan() {
        List<HDayView.EventTimeRange> timeRanges =
                Collections.singletonList(new HDayView.EventTimeRange(55, 133));

        HDayView.EventColumnSpansHelper columnSpansHelper =
                new HDayView.EventColumnSpansHelper(timeRanges);

        assertThat(columnSpansHelper.columnSpans.get(0).topColumn, is(0));
        assertThat(columnSpansHelper.columnSpans.get(0).BottomColumn, is(1));
    ...
    public void multipleEventColumnSpans() {
        List<HDayView.EventTimeRange> timeRanges = new ArrayList<>();
        timeRanges.add(new HDayView.EventTimeRange(30, 180));
        timeRanges.add(new HDayView.EventTimeRange(90, 120));
        timeRanges.add(new HDayView.EventTimeRange(150, 300));
        timeRanges.add(new HDayView.EventTimeRange(150, 300));
        ...
        assertThat(columnSpansHelper.columnSpans.get(1).topColumn, is(1));
        assertThat(columnSpansHelper.columnSpans.get(1).BottomColumn, is(3));

        assertThat(columnSpansHelper.columnSpans.get(2).topColumn, is(1));
        assertThat(columnSpansHelper.columnSpans.get(2).BottomColumn, is(2));

        assertThat(columnSpansHelper.columnSpans.get(3).topColumn, is(2));
        assertThat(columnSpansHelper.columnSpans.get(3).BottomColumn, is(3));
